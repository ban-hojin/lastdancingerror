...
// 1.1 함수 정의 반복  : getNews로 정리 
// 1.2. const getNews = async()=> {const response = await fetch(url); newList = data.articles; render()}
// 2. 긴 코드 전역변수로 전환 (url), 전역변수로 정하고 나머지 상황에 맞게 재정의
// 3. 함수 한번 정의하면 다음에 재활용 할 수 있다.


// 에러 처리 (사용자 전달 : 에러 핸들링, 에로가 왜 났는지 보여줘야)
// 1. try,catch 함수 사용 = 소스코드 쓴고 이안에서 에러가 발생하면 잡아준다.
// 1.1 try{//소스코드를 쓴다. 이안에 에러가 발생하면 }catch(){} 에러를 잡아준다.
// 2. console.log("내가 잡은 에로는",message) , try{ noona}catch(error){console,log("내가 잡은 에러는",error.message)
// 2.1 결과값 : 내가 잡은 에러는  noona is not defined
// 3. 응답값이 에러인 상황이면 catch 실행 
// 3.1 let weight = 45 try{ if(weight <30){ throw new Error ("당신은 너무 말랐어")}
// 3.1.1 throw new Error(data,message) catch(error) / let weight = 29, 결과값 내가 잡은 에러는 당신은 너무 말랐어
// 3.1.2 에러 발생 순간 후순위 출력값은 나오지 않는다.
// 3.1 status 401 에러인상황 / api 키값 오류
// 3.2 api 에러 상황 정리 , 번호 정리, 상황 구체적 정의
// 3.2.1 에러 메세지 응답 값 현출 
// 3.2.2 try 안에 데이터 현출 값을 넣어준다.
// 4. 에러 메세지 화면 전달 
// 4.1. if(response.status===200) 정상적인 상황인경우에는 현출 /ok인 상황 
// 4.2.  else{ three new Error(data, message)} 정상적이지 않으면 메세지 전달
// 5. 에러 메세지 보여주기 render (빨간색 음영) ("error", error.message)
// 6. 에러 메세지 화면 표시 
// 6.1 부트스트랩 빨간색 음영 표시 선정 복사/ 경고 메세지
// 6.2 const errorHTML = <div>a simple danger alert-check it out!</div>
// 7. 메세지는 상황에 따라 변화해야
// 7.0. 매개변수 활용 errorRender(error.message) 
// 7.1 <class="alert-danger" role=alert">
// 7.2 ${errorMessage}, errorRender(error.message)
// 7.3 뉴스를 치우고 보여주어야 함. error HTML, document.getElementId("news-board").innerHTML=errorHTML
// 8. 검색되지 않을 경우, 메세지 표시, 서치는 성공 (result==0일 경우), 에러는 아님
